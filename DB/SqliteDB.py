...
import pickle
import time
from ImageAnalyzationModule import ImageAnalyzationFile
...
class ImageDB:
    ...
    def searchImageByTerm(self, termName) -> list[ImageAnalyzationFile.ImageData]:
            ...
            for row in rows:
                ...
                image_objects[img_id].append(ImageAnalyzationFile.ImageClassificationData(termName, None, pickle.loads(row[2]),row[3]))
            print(time.time()-start)
            results: list[ImageAnalyzationFile.ImageData] = []
            for img_id in image_objects.keys():
                self.cursor.execute("SELECT i.* FROM images i WHERE i.id = ?", (img_id,))
                image = self.cursor.fetchone()
                results.append(ImageAnalyzationFile.ImageData(image[1], image_objects[img_id], pickle.loads(image[2])))
    ...
    def search_by_image(self, terms:list):
        ...
        for row in rows:
            ...
            if img_id not in image_objects:
                image_objects[img_id] = ImageAnalyzationFile.ImageData(img_path, [], pickle.loads(img_features))
                
            image_objects[img_id].classes.append(ImageAnalyzationFile.ImageClassificationData(class_name,None,pickle.loads(obj_features),weight))
